...
### D-066. 매칭 요청 반려(rejected) 상태 도입
- **배경**: 접수된 매칭 요청이 기준 미달·정보 부족이어도 운영자가 되돌릴 방법이 없었음(cancelled=광고주 취소, closed_lost=매칭 불발과 의미 구분 필요).
- **결정**: `request_status`에 `rejected` 추가(024_request_reject.sql — 021 번호가 페이백 021_payback_apply_light와 겹쳐 024로 재번호, `rejected_at`·`reject_reason` 컬럼 포함). 반려는 **후보 전달 전 단계(submitted~curating)에서만** 허용 — 후보가 광고주에게 전달된 뒤에는 기존 closed_lost 플로우 사용. 반려 시 사유 필수, 광고주에게 메일(matchingRequestRejectedEmail)+인앱 알림 통보. 반려·취소·마감된 요청에는 RFP 발송 차단. UI: 어드민 상세 반려 패널(2단계 확인), 목록 "반려" 탭, 보드 종료 칼럼, 광고주 상세 반려 안내(사유+재제출 유도), 타임라인 "요청 반려" 이벤트.
