...
### D-073. 솔루션 구독 메뉴 + 요금 페이지 (2026-09-16)
- **배경**: 페이백 없이 솔루션만 쓰려는 광고주용 유료 구독 경로 요구. 헤더 "광고비 페이백" 우측에 "솔루션 구독"(/solutions) 메뉴 추가(푸터 동일).
- **요금(VAT 별도)**: ① CatchLog 로그분석 — 월 PV 구간제 10만 17,500 / 20만 24,000 / 30만 30,500 / 40만 37,000, 40만 초과 10만 PV당 +6,500, 연간 요금 없음(연간 선택 시 월×12) ② 자동리포트 월 70,000 / 연 700,000 ③ 자동 ROAS 최적화(AUTO BID) 월 100,000 / 연 1,000,000.
- **번들 할인**: 2종 구독 시 전체 금액 20%, 3종 구독 시 40% (사용자 지시). 계산은 `lib/solution-pricing.ts` 단일 모듈(정수·floor, vitest 14케이스).
- **구독 문의**: 별도 테이블 없이 pb_leads 재사용 — `submitQuickLead`에 알림 라벨 옵션 추가, source `sub:{키}+{키}/M|Y[/pvNN]`(예: `sub:log+report+bid/M/pv10`), expected_budget에 예상 구독료 저장. 제출 시 Purchase/purchase 전환(content_name `solution_subscribe_*`, value=구독료).
- **페이백 크로스셀**: 하단에 "대행권만 지정하면 3종 무료" CTA(location `solutions_pricing`).

### D-074. 솔루션 설명 확장 — 소개 영상 자막 기반 (2026-09-16)
- **배경**: 랜딩 솔루션 섹션 설명이 2~3줄 요약에 그쳐 "자세히 알아보기" 팝업을 열어야만 기능을 알 수 있었음. 사용자가 유튜브 소개 영상 3편의 자막(.srt)을 제공하며 팝업 수준으로 섹션을 확장하도록 지시.
- **반영**: 메인 쇼케이스 각 솔루션 기능 6개 + 세부 기능 칩(highlights), 구독 페이지 카드 기능 6~8개로 확장. 내용은 자막의 실제 기능(CatchLog: 부정클릭 IP 차단·스마트스토어 추적용 URL·AI 사이트분석/성과진단/누수진단·상담 챗봇 / AUTO REPORT: 대시보드·일주월/맞춤 리포트·메일 자동발송·커스텀 시트·증액/감액/원클릭 계정분석 / AUTO BID: 소재ID·키워드별 조정·증감 비율·볼륨 보호·제외 조건·제품 분류 계수·원클릭 반영)만 사용, 수치 예시는 영상 설명 그대로.
- **레이아웃**: 텍스트 컬럼이 길어져 영상 컬럼은 md 이상에서 sticky(top-24)로 따라오게 변경.

### D-075. 솔루션 구독 페이지 재구성 + 메인 솔루션 섹션 원복 (2026-09-16)
- **배경**: D-074에서 메인 섹션을 길게 확장한 것은 오해 — 사용자 의도는 **구독 페이지**에서 솔루션별 상세 설명을 제공하는 것. 구독 페이지 1차안(3열 카드)은 가독성이 낮다는 피드백.
- **결정**: 메인 솔루션 섹션은 #67 이전(기능 2~3개) 버전으로 원복하고 "솔루션 자세히 알아보기" 버튼은 팝업 대신 `/solutions#{log|report|bid}`로 이동. 구독 페이지는 ① 상단 예상 구독료 박스(결제 주기·솔루션 선택 칩·PV·견적·문의) ② 솔루션별 상세 섹션(메인과 같은 설명+영상 레이아웃, 자막 기반 기능 6개+칩) ③ 각 섹션 하단 가격표(CatchLog: PV 구간표+초과 요금 / 나머지: 월·연 표, 포함 기능, "예상 구독료에 포함" 체크)로 재구성. 상세 데이터는 `components/solutions/solution-catalog.ts`로 분리.
- **후속 수정(사용자)**: CatchLog 14일 무료체험 문구 삭제. 히어로 문구 줄바꿈·VAT 별도 안내는 작은 글씨로 하단 배치.

### D-076. 솔루션 구독 페이지 — 구 팝업 상세 문서 인라인 + 섹션 구조 재정렬 (2026-09-16)
- **배경**: 사용자 의도는 "자세히 알아보기" 팝업에 있던 상세 소개 페이지(제공 원문 HTML 3종)를 구독 페이지의 각 솔루션에 그대로 배치하는 것. 추가 피드백 — 히어로 아래 회색 띠 제거, 히어로와 본문 사이 여백, 네이비 배경 불필요, 영상/가격표는 sticky 아님.
- **구조**: 각 솔루션 = 제목(중앙) → 소개 영상 + 가격표(2열, 고정) → 상세 소개 문서(`InlineHtmlDoc`: srcDoc iframe, sandbox 없이 same-origin으로 문서 높이를 읽어 자동 높이). 문서 내부 CTA의 postMessage(tf-open-apply)는 페이지 하단 ApplyCtaLink(id solution-modal-apply-cta)를 클릭해 간편 신청 팝업을 연다. 자막 기반 6개 기능 목록은 상세 문서로 대체(데이터는 solution-catalog.ts에 보존).

### D-077. 솔루션 구독 페이지 — 상세 소개 네이티브 전환 + 세부 UX 정리 (2026-09-16)
- **배경(사용자 피드백)**: ① 영상이 요금제 박스 높이 안에서 sticky로 움직임 → 상단 고정, 요금제 박스는 영상 높이에 맞춤 ② 상세 소개를 iframe(팝업 문서 그대로)으로 넣은 것은 의도와 다름 → 팝업의 **내용**을 페이지에 녹여 넣기 ③ 솔루션별 "페이백 신청하고 무료로 이용하기" 박스 제거 ④ 네이비 배경 불필요.
- **구현**: 팝업 HTML 3종(solution-details.ts)의 헤드라인·소개·칩·상세 기능 6·실행 로직 4·도입 효과 3을 `solution-catalog.ts` 구조화 데이터로 옮기고 라이트 테마 카드/스텝/스탯 컴포넌트로 렌더링(`**강조**` 마크업). `InlineHtmlDoc` 삭제. CatchLog 실행 로직 1단계는 구독 문맥에 맞게 "구독 문의 또는 페이백 신청 후 계정 발급(페이백 고객 무료)"로 조정 — 그 외 문구는 원문 유지.
- **레이아웃**: 영상 컬럼 `self-start`(고정), 요금제 카드는 행 높이에 stretch(영상보다 짧으면 영상 높이에 맞춤, 포함 기능 목록은 상세 소개로 이관해 카드 축소). 섹션 배경 white/muted 교차.
- **UX 추가**: 예상 구독료 박스 아래 솔루션 점프 내비, 박스가 화면 밖일 때 하단 고정 요약 바(N종·합계·구독 문의↑), 하단 페이백 크로스셀은 1회(네이비 밴드)만 유지.
- **리뷰 반영(적대적 리뷰 확정 7건)**: ① 헤드라인 `**강조**`는 extrabold navy 안에서 묻혀 브랜드 블루(`#004AAD`)로 구분 ② CatchLog 3번째 스탯의 "누적 데이터" 보조문구는 원문에 없어 삭제(sub 옵셔널) ③ 하단 크로스셀 밴드 `SectionHeader tone="dark"`(네이비 위 네이비 제목 → 보이지 않던 문제) ④ 점프 내비와 첫 섹션 사이 여백은 견적 섹션 자체 `pb`로 ⑤ 요약 바는 솔루션 섹션 끝 센티널을 지나면 숨겨 푸터를 가리지 않음 ⑥ 요약 바 문구는 truncate 대신 줄바꿈(모바일은 VAT 문구 생략) ⑦ 구독 문의 리드의 금액은 광고비가 아니므로 `lib/solution-pricing.ts`의 `encode/parseSubscriptionSource`로 source를 표준화하고 관리자 표·알림에 "구독료 / 월|연 (VAT 별도)"로 표시.

### D-078. 접수된 매칭 요청(RFP) 운영자 수정 기능 (2026-09-16)
- **배경**: 광고주가 제출 후 수정사항을 요청해도 시스템상 반영할 방법이 없었음(제출 폼은 광고주 전용, 제출 후 잠김).
- **결정**: `/admin/requests/{id}/edit` 운영자 수정 화면 추가. 광고주 제출과 **동일 스키마(matchingRequestSchema)로 검증**해 데이터 일관성 유지, title=브랜드명·budget_monthly=매체별 예산 합계 재계산 등 제출 액션과 동일 규칙 적용. 소진액 증빙 파일(ad_spend_proof)은 원본 보존(재업로드 없음). 저장 시 어드민 상세·목록, 광고주 상세, 파트너 RFP, RFP PDF 경로 일괄 revalidate. 상태 제한 없음(운영자 판단). 진입점은 어드민 요청 상세 우측 "✏️ 요청 수정".
- **비고**: RFP 재발송은 하지 않음 — 이미 발송된 대행사 화면·PDF는 같은 행을 읽으므로 즉시 최신 내용 노출. 대행사 재통보가 필요하면 기존 'RFP 재발송' 버튼 사용.

### D-079. 솔루션 구독 페이지 — CatchLog PV 드롭박스 위치·요금제 카드 드롭박스·연간 ×10 (2026-09-16)
- **사용자 요청**: ① 예상 구독료 박스의 CatchLog PV 드롭박스를 CatchLog 칩 바로 우측으로 ② CatchLog 요금제 카드의 PV별 표를 드롭박스로 ③ CatchLog 연간 결제는 월 요금 × 10 (2개월 무료).
- **구현**: `CATCHLOG_YEARLY_MONTHS = 10`을 `solutionPrice`에 적용(테스트 갱신). 요금제 카드는 3종 공통 "월간 / 연간(2개월 무료)" 표로 통일하고 CatchLog는 카드 상단 PV 드롭박스로 금액이 바뀜. 두 드롭박스는 같은 pv 상태를 공유해 어디서 바꿔도 견적에 반영.

### D-080. RFP 문서에서 광고주 PII 제거 — 업체명·사이트 주소까지만 공개 (2026-09-17)
- **배경(사용자 지시)**: "팀퍼스트 RFP에는 광고주 정보가 들어가선 안돼. 광고주 업체명과 사이트 주소까지만 오픈되게" — RFP PDF(인쇄) 화면의 '광고주 정보' 섹션이 사업자등록번호·대표자명·담당자(이름/직책)·연락처·이메일을 노출하고 있었음. RFP는 대행사에 전달되는 문서이므로 운영자/광고주 화면에서 출력한 PDF에도 PII가 실려서는 안 됨.
- **결정**: `RfpDocument`에서 '광고주 정보' 섹션과 `masked` prop을 제거 — 뷰어 역할과 무관하게 RFP 문서에는 PII를 렌더링하지 않음(마스킹 옵션이 아니라 원천 제거). 허용 정보인 상호(사업자명)는 '브랜드 및 서비스 소개' 섹션에 브랜드·카테고리·Website와 함께 2열 그리드로 통합.
- **비고**: 파트너 RFP 웹 뷰와 rfpArrivedEmail은 원래 브랜드·카테고리·Website 수준만 노출해 변경 불필요. 광고주 전체 정보는 어드민 요청 상세(`/admin/requests/{id}`)에서 계속 확인 가능.

### D-081. 운영자 콘솔 — 사이드바 서비스별 그룹핑 + 솔루션 구독 관리 메뉴 + 대시보드 재구성 (2026-09-17)
- **배경(사용자 피드백)**: 사이드바가 생성 순서대로 나열돼 "미팅·정산·마케터"가 어떤 서비스의 기능인지 알기 어렵고, 솔루션 구독 관련 운영 메뉴가 없으며, 대시보드가 페이백·매칭 중심이라 사업 라인 전체를 한눈에 보기 어려움.
- **사이드바**: `AppNav`에 그룹(`{label, items}`) 지원 추가(기존 평면 배열도 호환). 운영자 메뉴를 **광고비 페이백**(파이프라인·정산) / **솔루션 구독**(문의·고객) / **대행사 매칭**(매칭 요청·파트너·브랜드사·미팅·매칭 정산) / **마케터 매칭**(신청·관리) / **콘텐츠**(성공 사례)로 묶고 "정산"은 "매칭 정산"으로 명확화.
...
### D-086. 전 발신 메일 숨은 참조(BCC) 고정 (2026-09-22)
- **사용자 지시**: "앞으로 팀퍼스트에서 발신되는 메일에는 반드시 숨은 참조 rldbs06@naver.com 을 포함해서 발송".
- **구현**: 모든 메일이 거치는 단일 진입점 `lib/email/resend.ts sendEmail`에 `bcc` 추가 — 매칭·페이백·운영자 알림 등 전 경로에 일괄 적용. 기본값 `rldbs06@naver.com`, `RESEND_BCC_EMAIL` 환경변수로 교체 가능(빈 문자열이면 비활성화).
- **배경**: 발송 성공/실패가 DB에 기록되지 않아(candidate-actions 등에서 sendEmail 결과 미확인) "메일이 나갔는지" 검증이 Resend 대시보드 외엔 불가했음. BCC 사본이 발송 증빙·내용 확인 수단이 된다.

### D-087. 운영자 검수 모드 — 광고주 URL을 운영자 계정으로 그대로 열람 (2026-09-23)
- **배경(사용자 리포트)**: 광고주 안내 메일의 결과 페이지 링크(`/client/request/{id}`)를 운영자가 클릭하면 `requireRole('client')`가 운영자 대시보드로 리다이렉트해 메일에 실린 랜딩을 검수할 수 없었음.
- **결정**: `requireRole(role, { allowAdmin: true })` 옵션 추가 — 광고주 레이아웃에서만 운영자 통과를 허용하고, 통과 시 상단에 "운영자 검수 모드" 앰버 배너를 표시(광고주 작업 미실행 안내 + 운영자 콘솔 복귀 링크). 데이터 조회는 RLS admin_all로 자연 허용되며, 광고주 전용 서버 액션(관심 표시·분석권한·미팅 제안·소진액)은 기존 `role !== 'client'` 가드가 그대로 막아 오조작 불가. 어드민 요청 상세 헤더에 "👁 광고주 화면 보기" 진입 링크 추가.
- **비고**: 파트너 화면은 `getCurrentPartnerId()` 기반 스코프라 동일 옵션만으로는 동작하지 않아 이번 범위에서 제외(필요 시 별도 작업). 광고주 대시보드는 검수 모드에서 전체 요청이 보임(운영자 권한 범위와 동일).
